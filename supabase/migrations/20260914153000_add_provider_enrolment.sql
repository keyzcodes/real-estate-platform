...
create function public.enrol_current_user_as_provider()
returns boolean
language plpgsql
security definer
set search_path = ''
...
begin
    ...
    if current_account_status = 'suspended' then
        raise exception using
            errcode = '42501',
            message = 'Suspended profiles cannot enrol as property providers.';
    end if;
...
'Adds property_provider only to the authenticated caller. The function accepts no UUID or role arguments, rejects suspended profiles, and is safe to repeat.';
